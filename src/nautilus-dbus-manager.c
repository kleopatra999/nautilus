...
#include <libnautilus-private/nautilus-directory.c>
...
static void
trigger_show_folders_operation (char **uris,
                                const char *startup_id)
{
  ...
  if (uris == NULL || uris[0] == NULL || startup_id == NULL)
    {
      DEBUG ("Called 'ShowFolders' with NULL arguments, discarding");
      return;
    }
  
  for (idx = 0; uris[idx] != NULL; idx++)
    { 
	  source_file = g_file_new_for_uri (uris[idx]);
	  directory = nautilus_directory_get (source_file);
      if (NAUTILUS_IS_DIRECTORY (directory)) 
	      uris[idx] = nautilus_directory_get_uri (directory);
    }
...
static void
trigger_show_items_operation (char **uris,
                              const char *startup_id)
{
  application = nautilus_application_dup_singleton ();
  ...
  if (uris == NULL || uris[0] == NULL || startup_id == NULL)
    {
      DEBUG ("Called 'ShowItems' with NULL arguments, discarding");
      return;
    }
                    ...
                    gpointer user_data)
{
  ...
  if (g_strcmp0 (method_name, "CopyFile") == 0)
	...
	if (g_strcmp0 (method_name, "ShowFolders") == 0)
    {
	  ...
	  trigger_show_folders_operation (uris, startup_id);

	  DEBUG ("Called ShowFiles with startup_id %s and uri %s\n", startup_id, uris[0]);
	...
	if (g_strcmp0 (method_name, "ShowItems") == 0)
    {
	  ...
	  g_variant_get (parameters, "(^a&s&s)", &uris, &startup_id);
	  trigger_show_items_operation (uris, startup_id);

	  DEBUG ("Called ShowItems with startup_id %s and uri %s\n", startup_id, uris[0]);
	...
	if (g_strcmp0 (method_name, "ShowProperties") == 0)
    {
	  ...
	  g_variant_get (parameters, "(^a&s&s)", &uris, &startup_id);
	  trigger_show_properties_operation (uris, startup_id);
